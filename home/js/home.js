 ...
 * Handles all UI interactions and functionality for main control center
 */
...
class HomeUI {
    ...
    async loadAudioSettings() {
        try {
            ...
            if (durationSlider && durationValue) {
                ...
                durationValue.textContent = this.currentSettings.playbackDuration / 1000;
            }
    ...
    toggleAudioSection() {
        ...
        if (!audioSource || !defaultSection || !customSection) return;
    ...
    updateTicketCounts(queueACount = 0, queueBCount = 0, totalCount = 0) {
        const queueAElement = document.getElementById('queueACount');
        const queueBElement = document.getElementById('queueBCount');
        const totalElement = document.getElementById('totalCount');
        
        if (queueAElement) queueAElement.textContent = queueACount;
        if (queueBElement) queueBElement.textContent = queueBCount;
            ...
            chrome.storage.local.get(['lastPollAt'], (result) => {
                ...
                if (result.lastPollAt) {
                    ...
                    if (secondsRemaining <= 5) {
                        secondsRemaining = this.currentPollInterval * 60; // Reset for next cycle
                    }
                }
    ...
    updateCountdownDisplay(seconds) {
        ...
        if (countdownValue) {
            ...
            countdownValue.textContent = `${minutes}:${remainingSeconds.toString().padStart(2, '0')}`;
        }
        
        if (countdownProgress) {
            const percentage = (seconds / (this.currentPollInterval * 60)) * 100;
